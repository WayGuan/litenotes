...
`),(n=s.errors[0].params)!=null&&n.allowedValues&&console.log((r=s.errors[0].params)==null?void 0:r.allowedValues),f.default.exit(1)}}}async processPaths(){await Promise.all(v.default.map(this.paths,async e=>this.processPath(e)))}async processPath(e){await k.globFiles(e).then(t=>v.default.map(t,n=>B.default.relative(".",n))).then(t=>this.filterFiles(t)).then(this.fulFillFiles).then(t=>this.formatFiles(t))}static globFiles(e){return new Promise((t,n)=>{(0,ze.default)(e,(r,s)=>r?n(r):t(s))})}async filterFiles(e){if(this.ignoreFile==="")return e;let t=/^\.\.*/,n=v.default.filter(e,a=>t.test(B.default.relative(".",a))),r=v.default.xor(e,n),s=(0,We.default)().add(this.ignoreFile).filter(r);return v.default.concat(n,s)}static fulFillFiles(e){return this.targetFiles.push(e),Promise.resolve(e)}async formatFiles(e){await Promise.all(v.default.map(e,async t=>this.formatFile(t)))}async formatFile(e){await this.findTailwindConfig(e),await this.readRuntimeConfig(e),await Z(e).then(t=>Promise.resolve(t.toString("utf-8"))).then(t=>new j(this.options).formatContent(t)).then(t=>this.checkFormatted(e,t)).then(t=>this.writeToFile(e,t)).catch(t=>{this.handleError(e,t)})}async checkFormatted(e,t){this.printFormattedOutput(e,t);let n=H.default.readFileSync(e,"utf-8"),r=N(n),s=N(t),a=ve(e,r,s);return this.diffs.push(a),this.outputs.push(t),a.length>0&&((this.options.progress||this.options.write)&&f.default.stdout.write($.default.green("F")),this.options.checkFormatted&&(f.default.stdout.write(`${e}
`),f.default.exitCode=1),this.formattedFiles.push(e)),a.length===0&&(this.options.progress||this.options.write)&&f.default.stdout.write($.default.green(".")),Promise.resolve(t)}printFormattedOutput(e,t){this.options.write||this.options.checkFormatted||(f.default.stdout.write(`${t}`),v.default.last(this.paths)===e||v.default.last(this.targetFiles)===e)||(this.paths.length>1||this.targetFiles.length>1)&&f.default.stdout.write(`
`)}writeToFile(e,t){this.options.write&&(this.options.checkFormatted||t.length===0||v.default.isNull(t)||v.default.isEmpty(t)||H.default.writeFile(e,t,n=>{n&&(f.default.stdout.write(`${$.default.red(n.message)}
`),f.default.exit(1))}))}handleError(e,t){(this.options.progress||this.options.write)&&f.default.stdout.write($.default.red("E")),f.default.exitCode=1,this.errors.push({path:e,message:t.message,error:t})}printPreamble(){this.options.checkFormatted&&f.default.stdout.write(`Check formatting... 
...
`;f.default.stdout.write(e),f.default.stdout.write($.default.bold.green(`Fixed: F
`)),f.default.stdout.write($.default.bold.red(`Errors: E
`)),f.default.stdout.write($.default.bold("Not Changed: ")+$.default.bold.green(`.
`))}printFormattedFiles(){if(this.formattedFiles.length===0){this.options.checkFormatted&&f.default.stdout.write($.default.bold(`
...
`);return}f.default.stdout.write($.default.bold(`
Formatted Files: 
`)),v.default.each(this.formattedFiles,e=>f.default.stdout.write(`${$.default.bold(e)}
`))}printDifferences(){if(this.options.diff){if(f.default.stdout.write($.default.bold(`
Differences: 

`)),v.default.filter(this.diffs,e=>e.length>0).length===0){f.default.stdout.write((0,$.default)(`No changes found. 

`));return}v.default.each(this.diffs,e=>we(e))}}printErrors(){v.default.isEmpty(this.errors)||(f.default.stdout.write($.default.red.bold(`
Errors: 

`)),v.default.each(this.errors,e=>f.default.stdout.write(`${ae.default.format(e)}
`)))}};var le="blade-formatter";var Je="1.32.6";async function Ze(){let h=await(0,Xe.default)((0,Qe.hideBin)(process.argv)).usage(`${W.default.green(le)} ${Je}
...
`),process.stdout.write(`${W.default.yellow("Usage: ")} ${le} [options] [file glob | ...]
...
For more information, see https://github.com/shufo/blade-formatter`),e=await Ge.promises.readFile(require.resolve("vscode-oniguruma/release/onig.wasm"));await(0,Ue.loadWASM)(e.buffer);let t=Ve.default.chain(h.argv).set("noMultipleEmptyLines",!h.argv.multipleEmptyLines).set("noPhpSyntaxCheck",!h.argv.phpSyntaxCheck).value();if(h.argv.stdin){await process.stdin.pipe((0,Me.default)({encoding:"string"},r=>new k(t).format(r).then(s=>{s!==void 0&&process.stdout.write(s)}).catch(s=>{process.stdout.write(`${s.toString()}
`),process.exit(1)})));return}if(h.argv._.length===0){h.showHelp();return}await new k(t,h.argv._).formatFromCLI()}0&&(module.exports={});
